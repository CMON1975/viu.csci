...
//sort the user numbers from least to greatest (I did this for practice)
//void sortNumbers(int arraySize, float userNumbers[]);

//swap floats in array (I did this for practice, not used)
...
/*    if(arraySize>1){ //checking for more than one value to sort (practice, not used)
...
/* sort the user numbers from least to greatest, unused
void sortNumbers(int arraySize, float userNumbers[])
{
...
void printResults(int arraySize, float userNumbers[])
{
    ...
    for(int i=0,neg=0,pos=0;i<arraySize;i++){
        if(userNumbers[i]<0){
            negatives[neg] = userNumbers[i]; //fill negative value array
            neg++;
        }
        if(userNumbers[i]>0){
            positives[pos] = userNumbers[i]; //fill positive array
            pos++;
        }
    }
...
/*swap two results in float array (unused, just for practice)
void swap(float &a, float &b)
{
    float temp;
    temp = a;
