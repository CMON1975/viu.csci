...
// helper function to set parent pointers of child nodes
template<typename Key, typename Data>
void setParentPointers(Node<Key, Data>* parent, Node<Key, Data>* child) {
    if (child != nullptr) {
        child->parent = parent;
    }
}
...
void splitInternalNode(Node<Key, Data>* r, Key& k, Data*& dataptr, Node<Key, Data>* newChild, Node<Key, Data>*& newNode) {
    ...
    newNode->parent = r->parent;
    for (int i = 0; i <= newNode->size; i++) {
        setParentPointers(newNode, newNode->children[i]);
    }
    k = tempKeys[mid];
    dataptr = tempData[mid];
}
