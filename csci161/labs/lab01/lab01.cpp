...
//
// Notes: Always check while loops for unnecessary ";"
//
...
{
	Node *tmp = listHead; //temporary pointer
   //traverse the list, looking to match the incoming word
   while(tmp->next->data->word!=query)
   {
      tmp = tmp->next;
   }
   if(!tmp->next->data) return "DEBUG\n";
   else return tmp->next->data->definition;
}
